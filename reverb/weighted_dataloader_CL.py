...
class CanSeedDataset(Dataset):
    def __init__(self, pairs, nentity, nrelation, negative_sample_size, mode,ents,scoredict):
        self.len = len(pairs)
        self.pairs = pairs
    ...
    def __getitem__(self, idx):
        ...
        score = self.scoredict[(head, tail)]
        ...
        while negative_sample_size < self.negative_sample_size:
            if self.mode == 'head-batch':
                ...
                randlist = np.random.randint(low=0,high=28796, size=self.negative_sample_size * 2)
        ...
        return positive_sample, negative_sample, mode,score

    @staticmethod
    def collate_fn(data):
        positive_sample = torch.stack([_[0] for _ in data], dim=0)
        negative_sample = torch.stack([_[1] for _ in data], dim=0)
        score = torch.cat([_[3] for _ in data], dim=0)
        mode = data[0][2]
        return positive_sample, negative_sample,  mode,score
    ...
    def __getitem__(self, idx):
        ...
        while negative_sample_size < self.negative_sample_size:
            if self.mode == 'head-batch':
                ...
                randlist = np.random.randint(low=0,high=1770126, size=self.negative_sample_size * 2)
                randlist = randlist.tolist()
                negative_sample=np.array([self.ents[x] for x in randlist])
                mask = np.in1d(
                    negative_sample,
