...
def get_entseed_pair(ent_list, ent2id):
    ...
    with open('data/myexp/active_links_gjq.txt', "r", encoding='utf-8') as rf:
        ...
        ####### random #######
        # unused_head_np_names = list(set(list(head_np_links_dict.keys())) - set(active_head_np_names))
        # import random
        # random.seed(0)
        # active_head_np_names += random.sample(unused_head_np_names,
        #                                       int(len(head_np_links_dict) * 0.02) + 1)
        ######################
        seed_pair = [(ent2id[np_name], ent2id[head_np_links_dict[np_name]])
                     for np_name in active_head_np_names]
        score_dict = {seed: 1 for seed in seed_pair}

    with open('data/myexp/active_links_gjq.txt', "w", encoding='utf-8') as wf:
        wf.write("\n".join(active_head_np_names))

    with open('data/myexp/softlinkseed.txt', "r", encoding='utf-8') as rf:
        for line in rf.readlines():
            np_name, ent_name, score = line.strip('\n').split('#####')
            if np_name not in active_head_np_names:
                seed_pair.append((ent2id[np_name], ent2id[ent_name]))
                score_dict[(ent2id[np_name], ent2id[ent_name])] = float(score)
        return seed_pair, score_dict
